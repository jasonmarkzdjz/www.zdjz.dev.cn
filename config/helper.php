...
class helper {
    ...
    public static function checkSms($vcode,$mobile){
        if(!\Illuminate\Support\Facades\Cache::store('redis')->has(\library\Service\Common\CacheConst::SEND_MESSAGE_VR_LOGIN.$mobile) || unserialize(\Illuminate\Support\Facades\Cache::store('redis')->get(\library\Service\Common\CacheConst::SEND_MESSAGE_VR_LOGIN.$mobile)) !=$vcode){
            return false;
        }
        return true;
    }
    public static function getRechangeType($type){
        switch ($type){
            case \library\Service\Contst\Common\RechangeConst::WECHANT:
                return  \library\Service\Common\Contst\PayConst::wechatnative;
            case \library\Service\Contst\Common\RechangeConst::ALIPAY:
                return \library\Service\Common\Contst\PayConst::alipay;
            default :
                return 0;
        }
    }
    ...
    public static function base64EncodeImage ($image_file) {
        $image_file = storage_path('image').'/'.$image_file;

...
$messageExchangeName = 'message.exchange.news';
$messageQueneName = 'message.queue.news';

$messageBind_key = 'message.queue.new';
...
//创建一个fonmant交换器
$messageExchange = new \AMQPExchange($chann);
$messageExchange->setName($messageExchangeName);//交换器名称
$messageExchange->setType(AMQP_EX_TYPE_FANOUT);//交换器类型
$messageExchange->setFlags(AMQP_DURABLE);//是否持久化
$messageExchange->setFlags(AMQP_AUTODELETE);//是否自动删除  当所有队列和交换机器绑定到当前交换器上不在使用时，是否自动删除交换器 true：删除false：不删除
$messageExchange->declareExchange();//声明交换器


$quene = new \AMQPQueue($chann);
$quene->setName($messageQueneName);
...
$quene->declareQueue();
$quene->bind($messageExchangeName,$connmentBind_key);
$messageExchange->publish(json_encode(array('code'=>1,'message'=>'西安欢迎你!'.rand(99,99999))),$route_key,AMQP_MANDATORY,array('delivery_mode'=>2));
$chann->close();
?>